...
public static class DependencyExtensions
{
    ...
    public static async Task WaitForDatabaseAsync(string connectionString, ILogger logger, int maxRetries = 15, int retryDelaySeconds = 5)
    {
        ...
        for (int retry = 0; retry < maxRetries; retry++)
        {
            try
            {
                ...
                logger.LogInformation("Conexão com PostgreSQL estabelecida com sucesso após {Retry} tentativas!", retry + 1);
                return;
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Tentativa {Retry}/{MaxRetries} falhou. Tentando novamente em {Delay} segundos...", 
    ...
    public static async Task WaitForRedisAsync(string connectionString, ILogger logger, int maxRetries = 10, int retryDelaySeconds = 3)
    {
        ...
        for (int retry = 0; retry < maxRetries; retry++)
        {
            try
            {
                ...
                {
                    connection.Dispose();
                    logger.LogInformation("Conexão com Redis estabelecida com sucesso após {Retry} tentativas!", retry + 1);
                    return;
                }
                
                logger.LogWarning("Tentativa {Retry}/{MaxRetries}: Redis ainda não está conectado", retry + 1, maxRetries);
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Tentativa {Retry}/{MaxRetries} falhou. Tentando novamente em {Delay} segundos...", 
    ...
    public static async Task WaitForDependenciesAsync(IConfiguration configuration, ILogger logger)
    {
        ...
        {
            await WaitForRedisAsync(redisConnectionString, logger);
        }
